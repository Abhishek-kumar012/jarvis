#__________________J.A.R.V.I.S___________________
...
class MainThread(QThread):
    ...
    def run_jarvis(self):
        ...
        while True:
            ...
            # can be silent for max of 10mins
            ...
            elif ('open teams'in self.command) or ('open stream'in self.command) or('open outlook'in self.command)or('open cms portal'in self.command)or('open dashboard'in self.command):
            ...
            elif ('open calculator'in self.command) or ('open notepad'in self.command) or ('open online classes'in self.command) or ('open editor'in self.command)  or ('open media player'in self.command):
                self.OpenApp(self.command)

            #Command to close desktop applications
            elif ('close calculator'in self.command) or ('close notepad'in self.command) or ('close online classes'in self.command)    or ('close editor'in self.command) or ('close steam'in self.command) or ('close media player'in self.command):
                self.CloseApp(self.command)

            #command for opening shopping websites 
            elif ('flipkart'in self.command) or ('amazon'in self.command) or ('myntra' in self.command) :
            ...
            #Command for opening an instagram profile and downloading the profile pictures of the profil
            ...
            #command for opening your mobile camera
            ...
            #command for playing a dowloaded mp3 song in which is present in our system
            elif 'music' in self.command:
                try:
                    music_dir = 'C:\\Users\\devan\\Music'
            ...
            elif ("goodbye" in self.command) or ("get lost" in self.command) or ("exit" in self.command) or ("bye" in self.command):
    ...
    #Web camera
    # press "ESC" key to exit from web cam
    def webCam(self):    
    ...
    def Fun(self,command):
        ...
        if 'your name' in command:
            ...
            self.talk("your name is Abhishek")
        elif 'university name' in command:
            self.talk("you are studing in IPS Academy, with batcheloe in Computer Science and Enginnering") 
    ...
    def shedule(self):
        ...
        Week = {"monday" : "Boss from 9:45 to 10:35 you have Computer Network class , from 10:35 to 11:50 you have  SCientific Appititude class, from 11:30 to 12:20 you have Compiler Design lab, and then lunch",
        "tuesday" : "Boss from 9:45 to 11:25  you have computer Network class, from 11:30 to 12:30 you have Scientific Aptpititude,from 12:20 to 01:10 you have Machine Learning and Artificial Intelligence class, from 1:00 to 2:00 ,you have brake",
        "wednesday" : "Boss today you have a full day of classes, from 9:45 to 10:35 you have Compiler Design class, from 11:30 to 12:20 you have FAIML class, from 12:20 to 01:10 you have Scientific Appititude, from 1:00 to 2:00 you have brake",
        "thrusday" : "Boss today you have a full day of classes, from 9:45 to 10:35 you have  SCientific Appititude class , from 10:35 to 11:25 you have FAIML class, from 11:30 to 12:20 you have Computer Network class, from 1:00 to 2:00 you have brake",
        "friday" : "Boss today you have a full day of classes from 9:45 to 01:10 you have Compiler Design class,from and then you have brake",
        "saturday" : "Boss today is holiday but we can't say anything when they will bomb with any assisgnments",
    ...
    def college(self,command):
        ...
        elif 'cms portal' in command:
            self.talk('opening your ips academy management system')
            webbrowser.open('cms1.ipsacademy.net/')
        elif 'dashboard' in command:
            self.talk('opening dashboard fee portal')
            webbrowser.open('https://ipsacademy.net/index.php/')
            self.No_result_found()
    ...
    def OnlineClasses(self,command):
        ...
        if("code with harry" in command):
            self.talk('opening code with harry  on  youtube')
            webbrowser.open("https://www.youtube.com/results?search_query=code+with+harry")
        elif("machine learning" in command):
            self.talk('opening machine learning class on youtube')
            webbrowser.open("https://www.youtube.com/results?search_query=machine+learning+code+with+harry")
    ...
    def brows(self,command):
        ...
        if 'google' in command:
            ...
            #taking command for what to search in google
            S = self.take_Command()
            webbrowser.open(f"{S}")
    ...
    def shopping(self,command):
        ...
        if 'flipkart' in command:
            ...
            self.talk('opening myntra online shopping website')
    ...
    #News
    def news(self):
        MAIN_URL_= "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=yourapikey"
        MAIN_PAGE_ = get(MAIN_URL_).json()
        articles = MAIN_PAGE_["articles"]
        headings=[]
        seq = ['first','second','third','fourth','fifth','sixth','seventh','eighth','ninth','tenth'] #max 10 min 
        for ar in articles:
...
jarvis = Gui_start(path=current_path)
jarvis.show()
